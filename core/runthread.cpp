...
RunThread::RunThread () {
    ...
    connect(RunManager::ptr(),SIGNAL(emitRecordStatus(bool)),this,SLOT(receiveRecordSignal(bool)));
...
void RunThread::run()
{
    ...
    InterfaceManager::ptr()->getMainInterface()->writeA32D16(0xBB006090,3);
...
void RunThread::receiveRecordSignal(bool receivedRecordStatus)
{
...
bool RunThread::acquire()
{
    ...
    for (int i = 0; i < modulesz; ++i)
    {
        ...
        if (/*curM == _trg ||*/ curM->dataReady ()) {
            ...
            length=curM->acquire(ev);
            if(length<5)
            {
                problem=1;
                QDateTime time=QDateTime::currentDateTime();
                std::cout<<time.date().toString("dddd dd:MM:yyyy").toStdString()<<" "<<time.time().toString("HH:mm:ss").toStdString()<<" Problem at module "<<i<<" with data length "<<length<<std::endl;
            }
...
void RunThread::pollLoop()
{
    ...
    while(!abort)
    {
        ...
        if(nofPolls-lastAcqPoll>10000000)
            {
                lastAcqPoll=nofPolls;
                foreach (AbstractModule *m, modules) {
                    m->reset ();
