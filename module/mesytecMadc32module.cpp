...
int MesytecMadc32Module::configure () {
    ...
    if (ret) printf ("Error %d at MADC32V2_ADDR_SOURCE \n", ret);
    ...
    // set cblt mcst ctrl
    ret = iface->writeA32D16(baddr + MADC32V2_CBLT_MCST_CTRL, conf_.cblt_mcst_ctrl);
...
int MesytecMadc32Module::panicReset()
{
    return readoutReset();
}
...
void MesytecMadc32Module::counterResetSync()
{
    counterResetAll();
}
...
static const confmap_t confmap [] = {
    ...
    confmap_t ("cblt_active", &MesytecMadc32ModuleConfig::cblt_active),
    confmap_t ("mcst_active", &MesytecMadc32ModuleConfig::mcst_active),
...
int MesytecMadc32Module::getSettings()
{
    switch(conf_.data_length_format) {
    case MesytecMadc32ModuleConfig::dl8bit:
        return 8;
