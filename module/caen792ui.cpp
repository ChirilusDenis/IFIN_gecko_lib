...
QWidget* Caen792UI::createSettings2Controls()
{
    ...
    inactiveCBLT = new QRadioButton ("Inactive");
    firstCBLT = new QRadioButton ("First");
    intermediateCBLT = new QRadioButton ("Intermediate");
    lastCBLT = new QRadioButton ("Last");
    cbltAddr = new HexSpinBox (cbltbox);
    cbltAddr->setRange(0,255);
    QLabel *cbltLabel = new QLabel("CBLT Address");
    ...
    connect(inactiveCBLT,SIGNAL(toggled(bool)),this,SLOT(settings2Changed()));
    connect(firstCBLT,SIGNAL(toggled(bool)),this,SLOT(settings2Changed()));
    connect(intermediateCBLT,SIGNAL(toggled(bool)),this,SLOT(settings2Changed()));
    connect(lastCBLT,SIGNAL(toggled(bool)),this,SLOT(settings2Changed()));
    connect(cbltAddr,SIGNAL(valueChanged(int)),this,SLOT(settings2Changed()));
    ...
    l->addWidget(cbltLabel,       4,0,1,1);
    l->addWidget(cbltbox,         4,1,1,2);
    l->addWidget(inactiveCBLT,    5,0,1,1);
    l->addWidget(firstCBLT,       5,1,1,1);
    l->addWidget(intermediateCBLT,    5,2,1,1);
    l->addWidget(lastCBLT,        5,3,1,1);
...
void Caen792UI::settings2Changed()
{
    ...
    module->getConfig ()->cblt_addr                   = cbltAddr->value();
    if(firstCBLT->isChecked())
        module->getConfig ()->cblt_ctrl               = 2;
    else if(lastCBLT->isChecked())
        module->getConfig ()->cblt_ctrl               = 1;
    else if(intermediateCBLT->isChecked())
        module->getConfig ()->cblt_ctrl               = 3;
    else if(inactiveCBLT->isChecked())
        module->getConfig ()->cblt_ctrl               = 0;
...
void Caen792UI::applySettings()
{
    ...
    cbltAddr->setValue(module->getConfig()->cblt_addr);

    if(module->getConfig()->cblt_ctrl==0)
        inactiveCBLT->setChecked(1);
    else if (module->getConfig()->cblt_ctrl==1)
        lastCBLT->setChecked(1);
    else if (module->getConfig()->cblt_ctrl==2)
        firstCBLT->setChecked(1);
    else if (module->getConfig()->cblt_ctrl==3)
        intermediateCBLT->setChecked(1);

    std::cout<<module->getConfig()->cblt_ctrl<<std::endl;
