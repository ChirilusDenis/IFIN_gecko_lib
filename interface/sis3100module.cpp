...
int Sis3100Module::open()
{
    ...
    // Set disable VME IRQ
    s3100_control_write(m_device,SIS3104_IRQ,3);
...
int Sis3100Module::setOutput3(bool enable)
{
    int ret = 0;
    if(enable) ret =  s3100_control_write(m_device,SIS3104_IO,sis3104_set_lemo_out1);
    else ret =  s3100_control_write(m_device,SIS3104_IO,sis3104_clear_lemo_out1);
    return ret;
}

int Sis3100Module::readOutput1()
{
    return 0;
}
...
int Sis3100Module::readIRQStatus(){
    uint32_t opt_vme_interrupt_status = 0;
    s3100_control_read(m_device,SIS3104_IRQ,&opt_vme_interrupt_status);

    const uint32_t mask = 0x02000000;

    if((opt_vme_interrupt_status & mask)==33554432)
         return 1;
    else return 0;
}
...
int Sis3100Module::readA32BLT32FIFO(const uint32_t addr, uint32_t* dma_buffer,
                                uint32_t request_nof_words, uint32_t* got_nof_words)
{
    return sis3100_vme_A32BLT32FIFO_read(m_device, addr, dma_buffer, request_nof_words, got_nof_words);
}
