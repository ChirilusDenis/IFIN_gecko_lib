...
void EventBuilderBIGPlugin::startingUpdateTimer(int time)
{
    updateTimer->start(time);
}

void EventBuilderBIGPlugin::stoppingUpdateTimer()
{
    updateTimer->stop();
}

void EventBuilderBIGPlugin::startingResetTimer(int time)
{
    resetTimer->start(time);
}

void EventBuilderBIGPlugin::stoppingResetTimer()
{
    resetTimer->stop();
}

void EventBuilderBIGPlugin::startingTriggerToLogbook(int time)
{
    triggerToLogbook->start(time);
}

void EventBuilderBIGPlugin::stoppingTriggerToLogbook()
{
    triggerToLogbook->stop();
}

void EventBuilderBIGPlugin::startingTriggerToRunManager(int time)
{
    triggerToRunManager->start(time);
}

void EventBuilderBIGPlugin::stoppingTriggerToRunManager()
{
...
void EventBuilderBIGPlugin::showInfoBox()
{
...
void EventBuilderBIGPlugin::scriptNameButtonClicked()
{
     //Get configuration file
...
void EventBuilderBIGPlugin::infoInput()
{
    bool change=0;
...
void EventBuilderBIGPlugin::checkInfo()
{
...
void EventBuilderBIGPlugin::setScriptName(QString _scriptPath)
{
    ...
    do {
        ...
        for(uint16_t conf_entry = 0; conf_entry < numberOfDet; conf_entry++) {
            ...
            for(uint16_t param = 1; param < detchan[conf_entry].size() - 1; param++) {
                ...
                if(data[read_channel].size() >= read_idx[read_channel] + 2) {// Check if there is any remaining data to read on any channel
                    hasData = true;
        ...
        for(uint16_t conf_entry = 0; conf_entry < numberOfDet; conf_entry++) {
            ...
            // data_tree[detchan[conf_entry].back() - 1][0]->push_back(detchan[conf_entry][0]);
            ...
            for(uint16_t param = 1; param < detchan[conf_entry].size() - 1; param++) {
                ...
                if(data[read_channel].size() >= read_idx[read_channel] + 2) {
                    if(data[read_channel][read_idx[read_channel] + 1] < (leastTime + offset)) {
                        ...
                        // Skip the ts, used only to determine if a sensor fired in this event ???
                        big_branch[param][write_idx[det_type_idx]] = data[read_channel][read_idx[read_channel]];
                        // data_tree[detchan[conf_entry].back() - 1][param]->push_back(data[channel][read_idx[channel]++]);

                        read_idx[read_channel] += 2;

                    // } else {
                    //     data_tree[detchan[conf_entry].back() - 1][param]->push_back(0);
                    }
                }
            }
        ...
        if (local_bytes < 0) { // Some filling error ocurred
            printf("EventBuilderBIG: Error while writing to roottree!\n");
            break;
        }
